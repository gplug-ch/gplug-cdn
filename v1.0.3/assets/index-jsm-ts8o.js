...
`||r==="\r"?(c(),a=2):o+=r),a===3&&o==="!--"&&(a=4,f=f[0])}return c(),f})(e)),t),arguments,[])).length>1?t:t[0]}const u=Ci.bind(wr);var et={},Or="de",Wn={};function l(e,t){var n=et[e];return n===void 0?(Wn[e]||(Wn[e]=!0,console.warn('i18n: missing key "'+e+'"')),e):(t&&(n=n.replace(/\{(\w+)\}/g,function(r,i){return t[i]!==void 0?String(t[i]):r})),n)}function Si(e){var t=0;function n(){if(t>=e.length)return Promise.reject(new Error("i18n: no language file reachable"));var r=e[t++];return fetch(r).then(function(i){if(!i.ok)throw new Error("HTTP "+i.status);return i.json()}).then(function(i){return et=i||{},Or=et["meta.lang"]||document.documentElement.lang||"de",et}).catch(function(){return n()})}return n()}const Mi={t:l,load:Si,getLang:function(){return Or},getDict:function(){return et}};var nt=" ",Lr="−";function je(e,t){if(e==null||isNaN(e))return"–";var n=e<0,r=Math.abs(Number(e)).toFixed(t),i=r.split(".");return i[0]=i[0].replace(/\B(?=(\d{3})+(?!\d))/g,"'"),r=i.join("."),(n?Lr:"")+r}function Ni(e){return e.indexOf(".")>=0?e.replace(/\.?0+$/,""):e}function Ti(e){return e==null||isNaN(e)?"–":Math.abs(e)>=1e3?Ni(je(e/1e3,1))+nt+"kW":je(Math.round(e),0)+nt+"W"}function Ei(e){return e==null||isNaN(e)?"–":Math.abs(e)>=1e3?je(e/1e3,2)+nt+"kWh":je(Math.round(e),0)+nt+"Wh"}function Ii(e,t){if(e==null||isNaN(e))return"–";var n=je(e,2);return t&&e>0&&(n="+"+n),n+nt+"CHF"}function Te(e){return(e<10?"0":"")+e}function Pi(e,t){if(e==null)return"–";var n=new Date(e*1e3),r=Te(n.getDate())+"."+Te(n.getMonth()+1)+"."+n.getFullYear(),i=Te(n.getHours())+":"+Te(n.getMinutes());switch(t){case"1d":return r;case"dm":return Te(n.getDate())+"."+Te(n.getMonth()+1)+".";case"1mo":return Te(n.getMonth()+1)+"."+n.getFullYear();case"q":return n.getFullYear()+" Q"+(Math.floor(n.getMonth()/3)+1);case"hm":return i;case"15m":default:return r+" "+i}}const k={num:je,w:Ti,wh:Ei,chf:Ii,time:Pi,MINUS:Lr};function Me(e){return u`
      ...
      </header>`}function A(e){var t="card"+(e.group?" card-"+e.group:"")+(e.class?" "+e.class:"");return u`
      <section class=${t}>
      ...
      </div>`}function lt(e){var t=_e(null),n=Fr(t),r=x(null),i=r[0],a=r[1],o=e.height||180,s=e.series||[],f=e.timeWindow;if(!f){var c=1/0,d=-1/0;s.forEach(function(C){C.points.forEach(function(I){I.t<c&&(c=I.t),I.t>d&&(d=I.t)})}),f=c===1/0?[0,1]:[c,d]}var h=f[0],v=f[1]>f[0]?f[1]:f[0]+1,p=[];s.forEach(function(C){C.points.forEach(function(I){p.push(I.y)})});var g=Wr(p,!1),b=Math.max(10,n-K.left-K.right),$=o-K.top-K.bottom;function y(C){return K.left+(C-h)/(v-h)*b}function m(C){return K.top+(1-(C-g[0])/(g[1]-g[0]))*$}var w=Rr(g[0],g[1],4),M=e.xTickFormat||function(C){return k.time(C,"hm")},z=Fi(h,v,Math.max(2,Math.min(5,Math.floor(b/90)))).map(function(C){return{x:y(C),label:M(C)}});function E(C){var I=[],T=[];C.forEach(function(R){R.y===null||R.y===void 0?(T.length&&I.push(T),T=[]):T.push(R)}),T.length&&I.push(T);var B=m(Math.max(0,g[0]));return I.map(function(R){var _=R.map(function(W,D){return(D===0?"M":"L")+y(W.t).toFixed(1)+" "+m(W.y).toFixed(1)}).join(""),S=_+"L"+y(R[R.length-1].t).toFixed(1)+" "+B.toFixed(1)+"L"+y(R[0].t).toFixed(1)+" "+B.toFixed(1)+"Z";return{line:_,area:S}})}function U(C){if(t.current){var I=t.current.getBoundingClientRect(),T=C.clientX-I.left;if(T<K.left||T>n-K.right){a(null);return}var B=h+(T-K.left)/b*(v-h);a(B)}}var N=null,L=[];return i!==null&&(L.push({text:M(i)}),s.forEach(function(C){var I=An(C.points,i);I>=0&&(N===null&&(N=y(C.points[I].t)),L.push({color:C.color,text:(C.label?C.label+": ":"")+(e.yFormat||vn)(C.points[I].y)+(e.yFormat?"":" "+e.yUnit)}))}),N===null&&(N=y(i))),u`
      <div class="chart" ref=${t}
        onPointerMove=${U}
        onPointerLeave=${function(){a(null)}}>
        ${n>0?u`
          <svg width=${n} height=${o} role="img" aria-label=${e.label||""}>
            <${Ar} width=${n} height=${o} sy=${m}
              yTicks=${w} xTicks=${z}
              yUnit=${e.yUnit||"kW"} xUnit=${e.xUnit||"h"} />
            ${(e.bands||[]).map(function(C,I){return Ai(C.top,C.bottom,y,m).map(function(T,B){return u`<path key=${I+"-"+B} class="ch-band" d=${T} fill=${C.color} stroke="none"/>`})})}
            ...
            ${N!==null?u`
              ...
              ${s.map(function(C,I){var T=An(C.points,i);return T>=0?u`
                  <circle key=${I} cx=${y(C.points[T].t)} cy=${m(C.points[T].y)} r="3.5"
                    fill=${C.color} stroke="#fff" stroke-width="1.5"/>`:null})}`:null}
          </svg>
          ${N!==null?Dr(t,n,N,L):null}`:null}
      </div>`}function dn(e){var t=_e(null),n=Fr(t),r=x(null),i=r[0],a=r[1],o=e.height||180,s=e.points||[];function f(_){return(_.segments||[]).reduce(function(S,W){return S+(W.value||0)},0)}function c(_){return _.bars?_.bars.map(f):[_.y]}var d=s.reduce(function(_,S){return S.bars?Math.max(_,S.bars.length):_},1),h=[];s.forEach(function(_){c(_).forEach(function(S){h.push(S)})});var v=Wr(h,!0),p=Math.max(10,n-K.left-K.right),g=o-K.top-K.bottom;function b(_){return K.top+(1-(_-v[0])/(v[1]-v[0]))*g}var $=s.length,y=$>0?p/$:p;function m(_){return K.left+(_+.5)*y}var w=Math.max(1,Math.min(y-1,y*.72,46)),M=Rr(v[0],v[1],4),z=e.xTickFormat||function(_){return k.time(_,"hm")},E=Ri(s,p,z).map(function(_){return{x:m(_),label:z(s[_].t)}}),U=b(0),N=s.length+"|"+(s.length?s[0].t+"-"+s[s.length-1].t:"")+"|"+d+"|"+(e.yUnit||"");function L(_){if(!t.current||!$){a(null);return}var S=t.current.getBoundingClientRect(),W=_.clientX-S.left;if(W<K.left||W>n-K.right){a(null);return}var D=Math.floor((W-K.left)/y);a(D<0?0:D>=$?$-1:D)}var C=e.yFormat||vn;function I(_){return C(_)+(e.yFormat?"":" "+(e.yUnit||""))}var T=null,B=[];if(i!==null&&s[i]){var R=s[i];T=m(i),B.push({text:z(R.t)}),R.bars?R.bars.forEach(function(_){(_.segments||[]).forEach(function(S){S.value&&B.push({color:S.color,text:(S.label?S.label+": ":"")+I(S.value)})})}):R.y!==null&&R.y!==void 0&&B.push({color:R.color||e.color,text:I(R.y)})}return u`
      <div class="chart" ref=${t}
        onPointerMove=${L}
        onPointerLeave=${function(){a(null)}}>
        ${n>0?u`
          <svg width=${n} height=${o} role="img" aria-label=${e.label||""}>
            <${Ar} width=${n} height=${o} sy=${b}
              yTicks=${M} xTicks=${E}
              yUnit=${e.yUnit||"kWh"} xUnit=${e.xUnit||"t"} />
            <g class="ch-bars" key=${N}>
            ${s.map(function(_,S){var W=i===S?"1":"0.85";if(_.bars){var D=m(S)-w/2,ue=w/d;return u`<g key=${S}>${_.bars.map(function(fe,P){var V=D+P*ue+1,G=Math.max(1,ue-2),ne=0;return(fe.segments||[]).map(function(Fe,O){var ee=Fe.value||0;if(ee<=0)return null;var ae=b(ne+ee),oe=Math.max(1,b(ne)-ae);return ne+=ee,u`
                      <rect key=${P+"-"+O} class="ch-bar ch-bar-up"
                        x=${V.toFixed(1)} y=${ae.toFixed(1)}
                        width=${G.toFixed(1)} height=${oe.toFixed(1)} rx="1.5"
                        fill=${Fe.color} opacity=${W}/>`})})}</g>`}if(_.y===null||_.y===void 0)return null;var se=m(S)-w/2,j=b(_.y),X=Math.min(j,U),pe=Math.max(1,Math.abs(j-U));return u`
    ...
    `,burger:u`<svg viewBox="0 0 20 20" class="nav-icon" aria-hidden="true"><path d="M3 5h14M3 10h14M3 15h14" stroke="currentColor" stroke-width="2" stroke-linecap="round"/></svg>`};function Kn(){return u`
      ...
      </div>`}function za(e){var t=e.newest,n=e.win,r=e.power,i=e.vzevW,a=t?Oe(t.grid_w):null,o=t?Oe(t.pv_w)||0:null,s=t?Oe(t.bat_w)||0:null,f=t&&a!==null?o+s+a:null,c=t?o+s:null,d=t&&a!==null?Math.max(0,-a):null,h=t&&a!==null?Math.max(0,a):null,v=xa(r,n[0],n[1]),p=v.cons,g=v.prod;return u`
      <${A} group="grid" title=${l("panel.grid")}>
        <div class="ov-stats">
      ...
      <${A} group="production" title=${l("panel.production")}>
        ${e.notice?u`
      ...
      <${A} group="loads" title=${l("panel.loads")} tooltip=${l("tooltip.loads")}>
        <div class="ov-subgrid">
      ...
      <${A} group="vzev" title=${l("panel.vzev")} tooltip=${l("tooltip.vzev")}>
        <div class="ov-subgrid">
      ...
      <${A} group="grid" title=${l("flow.title")} tooltip=${l("tooltip.flow")}>
        ...
        <div class="flow-wrap">
          <svg class="flow-svg" viewBox="0 0 440 284" role="img"
            ...
            <g>
              ${v.map(function(y,m){var w=h(y.from),M=h(y.to);if(!w||!M)return null;var z=M.x-w.x,E=M.y-w.y,U=Math.sqrt(z*z+E*E)||1,N=z/U,L=E/U,C=w.x+N*Ee,I=w.y+L*Ee,T=M.x-N*Ee,B=M.y-L*Ee,R=y.state==="unknown",_=y.state==="zero",S=R?"var(--c-line)":Ia(y),W=R?5.5:7.5,D=T-N*(W+1.6),ue=B-L*(W+1.6),se=_?2:g(y.watts),j=((C+T)/2).toFixed(1),X=((I+B)/2-7).toFixed(1);return u`
...
`):l("tooltip.kpi_saving"),o=t.autarkyVzev!==null&&t.autarkyVzev!==void 0?l("kpi.autarky_vzev")+": "+r(t.autarkyVzev):null,s=!!e.showSaving,f=s?u`
      ...
      label=${l("kpi.selfuse")} tip=${n||l("tooltip.kpi_selfuse")} />`),e.showCo2&&c.push(u`<${jt} key="co" value=${Aa(t.co2Kg)} color="var(--c-production)"
      ...
      </div>`}function Aa(e){return e==null||isNaN(e)?"–":e>=1e3?k.num(e/1e3,2)+" t":k.num(e,e<10?2:1)+" kg"}function Da(){var e=x(null),t=e[0],n=e[1],r=x(null),i=r[0],a=r[1],o=x([]),s=o[0],f=o[1],c=x([]),d=c[0],h=c[1],v=x([]),p=v[0],g=v[1],b=x(!1),$=b[0],y=b[1],m=x(null),w=m[0],M=m[1],z=x(null),E=z[0],U=z[1],N=x(function(){return Math.floor(Date.now()/1e3)}),L=N[0],C=N[1],I=x(null),T=I[0],B=I[1],R=x({}),_=R[0],S=R[1],W=x(!1),D=W[0],ue=W[1],se=_e({}),j=_e({}),X=L,pe=X-Gn,fe=[pe,X];J(function(){F.getSite().then(function(q){n(q)}).catch(function(){}),F.getMeta().then(function(q){U(q)}).catch(function(){})},[]),J(function(){return F.poll(function(){F.getEnergy("15m",96).then(function(q){Array.isArray(q)&&M(q)}).catch(function(){})},6e4)},[]),J(function(){if(F.getVzevMembers)return F.poll(function(){F.getVzevMembers().then(function(q){Array.isArray(q)&&g(q)}).catch(function(){})},1e4)},[]),J(function(){var q=F.onVzevInfo(function(Ne){y(!!(Ne&&Ne.enabled))});return F.getVzevInfo().catch(function(){}),q},[]),J(function(){return F.poll(function(){var q=Math.floor(Date.now()/1e3);C(q);var Ne=q-Gn;Promise.all([F.getPower().catch(function(){return null}),F.getLoads().catch(function(){return null}),F.getProductions().catch(function(){return null})]).then(function(We){var At=We[0],Ke=We[1],Ye=We[2];At&&a(At),Ke&&f(Ke),Ye&&h(Ye),(At||Ke||Ye)&&B(q),ue(!0),Ye&&Ye.forEach(function(Ae){Xn(se.current,Ae.id,q,Tt(Ae),Ne)}),Ke&&Ke.forEach(function(Ae){var hi=bn(Ae.state)==="active"?Tt(Ae):0;Xn(j.current,Ae.id,q,hi,Ne)})})},1e4)},[]);var P=ka(i),V=Yr(i,pe,X),G=pa(p),ne=wa(P,s),Fe=ne&&!_[ne.key]?ne:null,O=!!(d&&d.length&&$a(V.pv.map(function(q){return q.y}))&&!_["flow.prod_nodata"]);function ee(){var q=Object.assign({},_);q["flow.prod_nodata"]=!0,S(q)}var ae=new Date;ae.setHours(0,0,0,0);var oe=Math.floor(ae.getTime()/1e3),Re=(w||[]).filter(function(q){return q.ts>=oe}),st=!!(p&&p.length),be=E&&E.tariffs||{},qe=be.co2_g_kwh===void 0||be.co2_g_kwh===null||be.co2_g_kwh===""?128:Number(be.co2_g_kwh),Rt=nn(Re,{tariffs:be,vzev:st,co2:qe}),Wt=Number(be.grid_import_chf_kwh)>0||Number(be.grid_feedin_chf_kwh)>0,ut=qe>0,si=Re.some(function(q){return q.partial}),ui=si?l("kpi.today_until",{time:k.time(L,"hm")}):l("kpi.today"),ci=(d||[]).some(function(q){return String(q.productionType||"").toUpperCase()==="BATTERY"}),fi=t&&t.location?t.location:null,vi=T?u`
      <span class="ov-updated">${l("common.stale",{time:k.time(T,"hm")})}</span>`:null;function di(q,Ne){F.setLoadState(q,Ne).then(function(){return F.getLoads()}).then(function(We){We&&f(We)}).catch(function(){le(l("error.toggle"),{type:"error"})})}function pi(){if(ne){var q=Object.assign({},_);q[ne.key]=!0,S(q)}}return u`
      <div>
        ...
        ${D?u`
          <${at}>
            ...
            <${Oa} newest=${P} vzevW=${G} hasBattery=${ci} records=${Re} />
            <${Ra} kpis=${Rt} period=${ui} showSaving=${Wt}
...
`)>=0||t.indexOf("\r")>=0?'"'+t.replace(/"/g,'""')+'"':t}function nr(e){return(e||[]).map(Ba).join(Zr)}function Qr(e,t){var n=[nr(e)];return(t||[]).forEach(function(r){n.push(nr(r))}),Ua+n.join(tr)+tr}function Ha(e,t){var n=new Date;function r(a){return(a<10?"0":"")+a}var i=n.getFullYear()+r(n.getMonth()+1)+r(n.getDate());return"gplug-verlauf-"+e+"-"+i+".csv"}var ja=[10,25,50],ge={"15m":{label:"history.res.15m",base:"15m",count:240,target:"15m",tk:"15m",slot:900},"1h":{label:"history.res.hour",base:"15m",count:240,target:"1h",tk:"15m",slot:3600},"1d":{label:"history.res.day",base:"1d",count:125,target:"1d",tk:"1d",slot:86400},"1w":{label:"history.res.week",base:"1d",count:125,target:"1w",tk:"1d",slot:604800},"1mo":{label:"history.res.month",base:"1mo",count:19,target:"1mo",tk:"1mo",slot:2592e3},"1q":{label:"history.res.quarter",base:"1mo",count:19,target:"1q",tk:"q",slot:7776e3}},Va=["15m","1h","1d","1w","1mo","1q"],qa={"15m":32,"1h":24,"1d":31,"1w":13,"1mo":13,"1q":9},ke={res:"15m",pageSize:25,chfMode:!1,chartMode:"net"};function Ka(e){return e==null?null:e/1e3}function Ya(e,t){var n;return t==="1mo"?(n=new Date(e*1e3),Math.floor(Date.UTC(n.getUTCFullYear(),n.getUTCMonth()-1,1)/1e3)):t==="1q"?(n=new Date(e*1e3),Math.floor(Date.UTC(n.getUTCFullYear(),n.getUTCMonth()-3,1)/1e3)):e-ge[t].slot}function Ga(e,t){if(!e.length)return[];var n=qa[t]||e.length,r={};e.forEach(function(f){r[f.ts]=f});for(var i=e[e.length-1].ts,a=[],o=e[0].ts,s=0;s<n&&o>=i;s++)a.push(r[o]||{ts:o,__blank:!0}),o=Ya(o,t);return a}function Vt(e,t){var n=Ka(e);return n===null?"–":k.num(n,2)}function Za(e){if(e==null)return u`<span>–</span>`;var t=e>0?"val-pos":e<0?"val-neg":"";return u`<span class=${t}>${k.chf(e,!0)}</span>`}function Qe(e,t){return e==null?u`<span>–</span>`:u`<span class=${t||""}>${k.chf(e,!1)}</span>`}function Qa(e,t,n){var r=[{key:"ts",label:l("history.col.time")},{key:"imp",label:l("history.col.gridimport"),unit:"[kWh]",align:"right"}];return n?(r.push({key:"impcost_ht",label:l("history.col.gridcost_ht"),unit:"[CHF]",align:"right"}),r.push({key:"impcost_nt",label:l("history.col.gridcost_nt"),unit:"[CHF]",align:"right"})):r.push({key:"impcost",label:l("history.col.gridcost"),unit:"[CHF]",align:"right"}),e&&t&&r.push({key:"exp",label:l("history.col.feedin"),unit:"[kWh]",align:"right"}),r.push({key:"vzev",label:l(e?"history.col.vzevexport":"history.col.vzevimport"),unit:"[kWh]",align:"right"}),r.push({key:"saldo",label:l("history.col.vzevsaldo"),unit:"[CHF]",align:"right"}),e&&(r.push({key:"saving",label:l("history.col.selfuse"),unit:"[CHF]",align:"right"}),r.push({key:"autarky",label:l("history.col.autarky"),unit:"[%]",align:"right"}),r.push({key:"selfuserate",label:l("history.col.selfuserate"),unit:"[%]",align:"right"}),r.push({key:"ersparnis",label:l("history.col.ersparnis"),unit:"[CHF]",align:"right"})),r}function rr(e){return e==null?"–":k.num(e*100,0)+" %"}function yn(e){var t=e.revenue_vzev_chf,n=e.cost_vzev_chf;return t===null&&n===null?null:(t||0)-(n||0)}function Xa(e,t,n){for(var r=ge[t].slot,i=[],a=0;a<e.length;a++){i.push({kind:"row",rec:e[a]});var o=e[a+1];if(o){var s=Math.round((e[a].ts-o.ts)/r)-1;s>=1&&i.push({kind:"gap",key:"g"+e[a].ts})}}return!n&&e.length&&i.push({kind:"boundary",key:"b"+e[e.length-1].ts}),i}function Ja(){var e=x(ke.res),t=e[0],n=e[1],r=x(0),i=r[0],a=r[1],o=x(ke.pageSize),s=o[0],f=o[1],c=x(ke.chfMode),d=c[0],h=c[1],v=x(ke.chartMode),p=v[0],g=v[1],b=x({records:null,tariffs:{},producer:!1,base15m:null,err:!1}),$=b[0],y=b[1],m=x(!0),w=m[0],M=m[1];ke.res=t,ke.pageSize=s,ke.chfMode=d,ke.chartMode=p,J(function(){var O=!1;M(!0);var ee=ge[t];return Promise.all([F.getEnergy(ee.base,ee.count).catch(function(){return null}),F.getMeta().catch(function(){return null}),F.getProductions().catch(function(){return null}),ee.base==="15m"?Promise.resolve(null):F.getEnergy("15m",240).catch(function(){return null})]).then(function(ae){if(!O){var oe=ae[0],Re=ae[1],st=ae[2],be=ae[3];if(oe===null){y({records:null,tariffs:{},producer:!1,base15m:null,err:!0}),M(!1);return}var qe=Re&&Re.tariffs||{},Rt=Array.isArray(st)&&st.some(function(ut){return ut&&ut.productionType==="PHOTOVOLTAIC"}),Wt=ee.base==="15m"?oe:be||null;y({records:oe,tariffs:qe,producer:Rt,base15m:Wt,err:!1}),M(!1)}}),function(){O=!0}},[t]);var z=Ft($.tariffs)&&ge[t].base==="15m",E=Ie(function(){if(!$.records)return[];var O=$.records;z&&(O=$.records.map(function(ae){return Hr(ae,$.tariffs)}));var ee=Dn(O,ge[t].base,ge[t].target,$.tariffs);return ee.slice().sort(function(ae,oe){return oe.ts-ae.ts})},[$.records,$.tariffs,t,z]),U=Ie(function(){if(t!=="15m"||!$.base15m)return{};var O=Dn($.base15m,"15m","15m",$.tariffs),ee=Xi(O,3),ae={};return ee.forEach(function(oe){ae[O[oe].ts]=!0}),ae},[$.base15m,$.tariffs,t]),N=E.some(function(O){return(O.exp_wh||0)>0}),L=Qa($.producer,N,z),C=E.some(function(O){return(O.vzev_in_wh||0)>0||(O.vzev_out_wh||0)>0}),I=$.tariffs.co2_g_kwh===void 0||$.tariffs.co2_g_kwh===null||$.tariffs.co2_g_kwh===""?128:Number($.tariffs.co2_g_kwh),T={tariffs:$.tariffs,vzev:C,co2:I},B=Number($.tariffs.grid_import_chf_kwh)>0||Number($.tariffs.grid_feedin_chf_kwh)>0,R=Ie(function(){var O={};return E.forEach(function(ee){O[ee.ts]=nn([ee],T)}),O},[E,$.tariffs,C,I]),_=Ie(function(){return nn(E,T)},[E,$.tariffs,C,I]),S=$.records?$.records.length<ge[t].count:!0,W=Xa(E,t,S),D=E.length;W.filter(function(O){return O.kind==="row"});var ue=Math.max(1,Math.ceil(D/s)),se=Math.min(i,ue-1),j=se*s,X=Math.min(j+s,D),pe=el(W,j,X);function fe(O){n(O),a(0)}function P(O){f(+O),a(0)}function V(){var O=L.map(function(oe){return oe.label+(oe.unit?" "+oe.unit:"")}),ee=E.map(function(oe){return rl(oe,L,t,$.producer,R[oe.ts])}),ae=Qr(O,ee);ul(Ha(t),ae)}var G=Ie(function(){return al(Ga(E,t),p,d,$.producer)},[E,p,d,$.producer,t]),ne=Ie(function(){return ll(E,t,$.producer)},[E,t,$.producer]),Fe=u`
      ...
      <div>
        ...
        ${$.err?u`
          <${A}><p class="placeholder-text">${l("common.nodata")}</p><//>`:null}

        ${!$.err&&w?u`
          <${A}><p class="placeholder-text">${l("common.loading")}</p><//>`:null}

        ${!$.err&&!w?u`
          <div>
            ${E.length?u`<${sl} summary=${ne} kpis=${_} showSaving=${B} />`:null}

            <${A} group="vzev" title=${l("history.chart.title")}>
              ...
              ${G.points.length?u`
                <${dn} height=${220}
                  ...
                  yFormat=${G.yFormat}
                  xTickFormat=${function(O){return il(t,O)}}
                  points=${G.points} />
            ...
            <${A} title=${l("history.table.title")}>
              <div class="table-wrap">
                <table class="table verlauf-table">
                  ...
                  <tbody>
                    ${D===0?u`
                      <tr><td class="table-empty" colspan=${L.length}>${l("common.nodata")}</td></tr>`:pe.map(function(O){return tl(O,L,t,U,$.producer,R)})}
                  </tbody>
                </table>
                  ...
                  <label class="table-pagesize">
                    ...
                    <span class="select-wrap select-wrap-small">
                      <select class="select select-small" value=${s}
                        ...
                        ${ja.map(function(O){return u`<option key=${O} value=${O}>${O}</option>`})}
                      </select>
                      <svg class="select-caret" viewBox="0 0 12 8" aria-hidden="true"><path d="M1 1.5 6 6.5 11 1.5" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round"/></svg>
                    </span>
                  </label>
      ...
      <${A} group="grid" title=${l("meter.section.power")}
      ...
      <${A} title=${l("meter.section.phases")} tooltip=${l("meter.tip.phases")}>
      ...
      <${A} group="production" title=${l("meter.section.registers")}
      ...
      <${A} title=${l("meter.section.minmax")} subtitle=${l("meter.since_open")}
      ...
      <div>
        ...
        ${t===void 0?u`<${A}><p class="placeholder-text">${l("common.loading")}</p><//>`:null}
        ${t===null?u`<${A}><p class="placeholder-text">${l("meter.empty")}</p><//>`:null}
        ${t?u`
          <${wl} resolved=${g} />
          <${xl} resolved=${g} />
          <${zl} resolved=${g} />
      ...
      </span>`}function Wl(){var e=x(null),t=e[0],n=e[1],r=x({}),i=r[0],a=r[1],o=x([]),s=o[0],f=o[1],c=x(!1),d=c[0],h=c[1],v=x(null),p=v[0],g=v[1],b=x(sr),$=b[0],y=b[1],m=x(null),w=m[0],M=m[1],z=_e(!0);function E(){F.getVzevMembersList().then(function(P){z.current&&(n(P),h(!1))}).catch(function(){z.current&&(n([]),h(!0))}),F.getVzevRaw().then(function(P){z.current&&g(P||null)}).catch(function(){z.current&&g(null)})}J(function(){z.current=!0,E(),F.getVzevDiscovered().then(function(V){z.current&&f(V)}).catch(function(){z.current&&f([])});var P=F.poll(function(){F.getVzevFlows("15m",1).then(function(V){z.current&&a(Fl(V&&V.flows))}).catch(function(){})},2e3);return function(){z.current=!1,P()}},[]);function U(){sr=!0,y(!0)}function N(P){F.get("/api/vzev/members?action=remove&id="+encodeURIComponent(P.id)).then(function(){le(l("vzev.removed"),{type:"info"}),E()}).catch(function(){le(l("vzev.saveerror"),{type:"error"})})}var L=!!(p&&p.self_id!=null&&p.self_id===p.producer_id),C=L?u`
      <${Y} secondary onClick=${function(){Ce.navigate("/vzev/abrechnung")}}>
        ${l("vzev.billing")} ›
      <//>`:null,I=u`<${Me} title=${l("page.vzev")} subtitle=${l("vzev.subtitle")} actions=${C} />`;if(t===null)return u`<div>${I}<${A}><p class="placeholder-text">${l("common.loading")}</p><//></div>`;var T=null,B=[];t.forEach(function(P){P.own||P.is_own?T=P:B.push(P)}),t.forEach(function(P){Xe(P)});var R=u`
      <${A} class="vz-discovery">
      ...
      <//>`;function _(P){var V=i[P.id];return typeof V=="number"?V:null}var S=0;if(p&&p.data)for(var W in p.data){var D=jn(p,W);D&&D>S&&(S=D)}function ue(P){if(!p||!p.data)return null;var V=jn(p,P.id),G=S||Math.floor(Date.now()/1e3);return V===null?{lastTs:null}:G-V>Il?{lastTs:V}:null}var se=$?null:u`
      <${A} class="vz-info">
      ...
      <${A} class="vz-drill">
        ...
        <${A} group="vzev" class="vz-graph-card">
        ...
        <${A} class="vz-form-card">
          <div class="vz-form">
            ...
            <div class="vz-form-actions">
              <${Y} onClick=${W} disabled=${I}>${l("vzev.form.save")}<//>
      ...
      <${A} group="vzev" title=${e.title} subtitle=${e.subtitle}>
      ...
      <div>
        ...
        <div class="settings-form">
          ...
          <${Le} label=${l("settings.load.type")} value=${t.loadType||ln[0]}
          ...
          <${Le} label=${l("settings.integration")} value=${o}
      ...
      <div>
        ...
        <div class="settings-form">
          ...
          <${Le} label=${l("settings.prod.type")} value=${t.productionType||It[0]}
          ...
          <${Le} label=${l("settings.dimension")} value=${t.dimension||"W"}
            options=${Ve(ri)} onChange=${r("dimension")} />
          <${Le} label=${l("settings.integration")} value=${i}
      ...
      <//>`:u`<${A} group="vzev"><p class="placeholder-text">${l("settings.loading")}</p><//>`}function gr(e){var t={};return["ssid1","password1","ssid2","password2"].forEach(function(n){!ie(e[n])&&String(e[n]).indexOf(";")>=0&&(t[n]="settings.err.wifi_semicolon")}),t}function $o(e){var t=e.net,n=t.RSSI!==void 0?t.RSSI+"%":t.Signal!==void 0?t.Signal+" dBm":"";return u`
      ...
      </div>`}function _o(){var e=x(null),t=e[0],n=e[1],r=x(!1),i=r[0],a=r[1],o=x(!1),s=o[0],f=o[1],c=x(!1),d=c[0],h=c[1],v=x({scanning:!1,networks:null,error:!1}),p=v[0],g=v[1];function b(){a(!1),n(null),F.getWifiConfig().then(function(E){n({ssid1:E.ssid1,password1:"",ssid2:E.ssid2,password2:""})}).catch(function(){a(!0)})}J(function(){b()},[]);function $(E){return function(U){n(function(N){var L=Object.assign({},N);return L[E]=U,L})}}function y(){d||!window.confirm(l("settings.gplug.restart_confirm"))||(h(!0),F.restartDevice().then(function(){le(l("settings.gplug.restart_success"),{type:"info"})}).catch(function(){le(l("settings.gplug.restart_error"),{type:"error"})}).then(function(){h(!1)}))}function m(){s||!t||it(gr(t))||(f(!0),F.setWifiConfig(t).then(function(){le(l("settings.gplug.wifi_saved"),{type:"info"})}).catch(function(E){le(E&&E.message||l("settings.save_error"),{type:"error"})}).then(function(){f(!1)}))}function w(){g({scanning:!0,networks:null,error:!1}),F.wifiScanStart().catch(function(){});var E=0;function U(){E+=1,F.wifiScanResult().then(function(N){var L=N&&(N.WiFiScan!==void 0?N.WiFiScan:N.WifiScan);if(L&&typeof L=="object"){var C=Object.keys(L).map(function(I){return L[I]});C.sort(function(I,T){return(Number(T.RSSI)||0)-(Number(I.RSSI)||0)}),g({scanning:!1,networks:C,error:!1});return}if(E>=10){g({scanning:!1,networks:null,error:!0});return}setTimeout(U,1e3)}).catch(function(){g({scanning:!1,networks:null,error:!0})})}setTimeout(U,1e3)}function M(E,U){n(function(N){var L=Object.assign({},N);return L["ssid"+E]=U,L})}if(i)return u`
        <${A} group="grid">
          <p class="placeholder-text">${l("settings.load_error")}</p>
          <${Y} secondary small onClick=${b}>${l("settings.retry")}<//>
        <//>`;if(!t)return u`<${A} group="grid"><p class="placeholder-text">${l("settings.loading")}</p><//>`;var z=gr(t);return u`
      <div>
        <${A} group="grid" title=${l("settings.gplug.restart_title")}>
        ...
        <${A} group="grid" title=${l("settings.gplug.wifi_title")}>
        ...
        <//>`:n===null?u`<${A} group="grid"><p class="placeholder-text">${l("settings.loading")}</p><//>`:u`
      <${A} group="grid">
      ...
      <//>`}function wo(){return{id:"",friendlyName:"",loadType:ln[0],currentPower:"",priority:1,integration:"simulator",url:""}}function ko(){return{id:"",friendlyName:"",productionType:It[0],dimension:"W",integration:"simulator",url:""}}function xo(e){var t=e.params&&e.params.tab||"site",n=eo[t]||"site",r=x(null),i=r[0],a=r[1],o=x(null),s=o[0],f=o[1],c=x(!1),d=c[0],h=c[1],v=x(!1),p=v[0],g=v[1],b=x({loads:[],productions:[],grid:[]}),$=b[0],y=b[1];function m(_){return(Array.isArray(_)?_:[]).map(function(S){return String(S&&S.id)}).filter(function(S){return!ie(S)})}function w(){f(null),F.getConfig().then(function(_){a(M(_)),g(!ie(_&&_.id)),y({loads:m(_&&_.loads),productions:m(_&&_.productions),grid:m(_&&_.grid)})}).catch(function(){f(!0),a(null)})}J(function(){w()},[]);function M(_){_=_||{};var S=Object.assign({},_);return S.loads=Array.isArray(_.loads)?_.loads:[],S.productions=Array.isArray(_.productions)?_.productions:[],S.grid=Array.isArray(_.grid)?_.grid:[],S.tariffs=Object.assign({},lo,_.tariffs||{}),S}function z(_,S){a(function(W){var D=Object.assign({},W);return D[_]=S,D})}function E(_,S){a(function(W){var D=Object.assign({},W);return D.tariffs=Object.assign({},W.tariffs),D.tariffs[_]=S,D})}function U(_){a(function(S){var W=Object.assign({},S);return W.tariffs=Object.assign({},S.tariffs),W.tariffs.ht_windows=_,W})}function N(_,S){a(function(W){var D=Object.assign({},W);return D[_]=S,D})}function L(){d||(h(!0),F.postConfig(C(i)).then(function(){le(l("settings.saved"),{type:"info"}),g(!0),w()}).catch(function(_){le(_&&_.message||l("settings.save_error"),{type:"error"})}).then(function(){h(!1)}))}function C(_){var S=Object.assign({},_);return S.loads=_.loads.map(function(W){var D=Object.assign({},W);return D.currentPower!==""&&D.currentPower!==void 0&&(D.currentPower=Number(D.currentPower)),D.priority!==""&&D.priority!==void 0&&(D.priority=Number(D.priority)),D}),S.tariffs=Object.assign({},_.tariffs),ii.forEach(function(W){S.tariffs[W]=Number(_.tariffs[W])}),_.tariffs.co2_g_kwh!==void 0&&_.tariffs.co2_g_kwh!==""&&(S.tariffs.co2_g_kwh=Number(_.tariffs.co2_g_kwh)),["grid_import_ht_chf_kwh","grid_import_nt_chf_kwh"].forEach(function(W){ie(_.tariffs[W])?delete S.tariffs[W]:S.tariffs[W]=Number(_.tariffs[W])}),Array.isArray(_.tariffs.ht_windows)&&(_.tariffs.ht_windows.length===0?delete S.tariffs.ht_windows:S.tariffs.ht_windows=_.tariffs.ht_windows.map(function(W){return{days:W.days,from:Number(W.from),to:Number(W.to)}})),S}function I(_){!_||ie(_.id)||F.setLoadState(_.id,"INACTIVE").catch(function(){})}function T(_){Ce.navigate("/einstellungen/"+to[_])}var B=Jl.map(function(_){return u`
        ...
        onSave=${L} saving=${d} />`:n==="tariffs"?R=u`<${ho} tariffs=${i.tariffs} patch=${E}
        setWindows=${U} onSave=${L} saving=${d} />`:n==="vzev"?R=u`<${mo} />`:n==="gplug"?R=u`<${_o} />`:n==="pro"?R=u`<${yo} onSaved=${w} />`:n==="loads"?R=u`<${Yt} group="loads"
        ...
        onSave=${L} saving=${d} />`:n==="productions"?R=u`<${Yt} group="production"
        ...
        onSave=${L} saving=${d} />`:n==="grid"&&(R=u`<${Yt} group="grid" fixedList
    ...
    <div>
      ...
      <${A} group="vzev" title="BarChart" subtitle="0-Achse, signierte Werte"
      ...
      <${A} group="loads" title="Badges & Buttons">
        <div class="demo-row">
          <${_t} state="active" />
          <${_t} state="waiting" />
          <${_t} state="inactive" />
      ...
      <${A} group="production" title="DataTable">
        <${Ui} pageSize=${10}
    ...
    </div>`}function Co(){return[{path:"/",component:Da},{path:"/verlauf",component:Ja},{path:"/zaehler",component:Ml},{path:"/vzev/mitglied/:id?",component:Ul},{path:"/vzev/abrechnung",component:Xl},{path:"/vzev",component:Wl},{path:"/einstellungen/:tab?",component:xo},{path:"/demo",component:zo}]}function mr(){var e=document.getElementById("app"),t="https://gplug.pages.dev/v1.0.3/lang.json";Mi.load([t,"/fs?name=lang.json","lang.json","i18n/de.json"]).catch(function(){}).then(function(){wi(u`<${ca} routes=${Co()} />`,e)})}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",mr):mr();
